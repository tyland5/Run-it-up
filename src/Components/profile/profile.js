...
function ProfileHeader({uid}){
    ...
    const getProfileInfo = async () =>{
        const res = await fetch(envVariables.serverURL + "/user/getUserInfo?" + new URLSearchParams({uid: uid}));
    ...
    const followUser = async() =>{
        ...
        const res = await fetch(envVariables.serverURL +"/user/followUser", {
            ...
            body: JSON.stringify({followingId: uid})
        })
    ...
    <View style={styles.followMetrics}>
        <TouchableWithoutFeedback onPress={() => navigation.push("FollowPage", {activeRouteIndex: 0, uid:uid})}>
            <StyledText small>{profileInfo.followers} Followers</StyledText>
        </TouchableWithoutFeedback>

        <TouchableWithoutFeedback onPress={() => navigation.push("FollowPage", {activeRouteIndex: 1, uid:uid})}>
...
// I need this or going to edit profile and doing show more or show less of bio triggers rerender of post list
// This is because Tabs container itself gets rerendered. Memo helps children not get rerendered even if parent does
const MemoPostList = memo(PostList, (oldprops, newprops)=>{ return (oldprops.filters.uid === newprops.filters.uid) })
export default function Profile({route}){
    ...
    <View style={styles.container}>
        <Tabs.Container
        renderHeader={useCallback(() => {return(<ProfileHeader uid = {route.params.uid} />)}, [])}
        renderTabBar={useCallback((props) => <MaterialTabBar {...props} indicatorStyle={{backgroundColor:'orange'}} activeColor="white" inactiveColor="gray" style={{backgroundColor:"#121212"}} />, [])}
        ...
        >
            <Tabs.Tab name="Posts">
                ...
                data={[[]]}
                renderItem={({item})=>(
                    <MemoPostList filters={{uid : route.params.uid}}/>
                )}/>
            </Tabs.Tab>
